...
class CacheProcessor:
    ...
    def insert_cache(self, colorname: str, rgba: tuple[float, float, float, float]):
        ...
        if colorname == None or type(colorname) != str or len(colorname) <=1:
            raise Exception("InvalidColorNameError", "colorname must be a string with length > 1")
        if len(rgba) != 4:
        ...
        self.cursor.execute(f"INSERT INTO {self.table} (COLOR, RED, GREEN, BLUE, ALPHA) VALUES (?,?,?,?,?)",
