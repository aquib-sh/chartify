...
from tools.collision_detector import CollisionDetectorExtended
...
class ChartifyAppExtended(tk.Tk):
    ...
    def save_file(self):
        ...
        if self.current_file_name.lower().endswith(".csv"):
            ...
            messagebox.showerror(
                "No name specified", "Data sucessfully saved to Output.csv"
            )
    ...
    def convert_timeunit(self, d: datetime.datetime) -> int:
        ...
        if self.duration_dtype == "Week":
            ...
            return (
                d.components.days * 24 * 60
                + d.components.hours * 60
                + d.components.minutes
            )
        elif self.duration_dtype == "Second":
            return (
                d.components.days * 24 * 60 * 60
                + d.components.hours * 60 * 60
                + d.components.minutes * 60
                + d.components.seconds
            )

    def generate_timeseries_xaxis(
        self, end: int, step: int, label_format: str, min: datetime.timedelta
    ) -> list:
        ...
        for i in range(1, end + 1, step):
            ...
            label = label_format.format(
                yyyy=year, mm=month, dd=day, hh=hour, _mm=minute, ss=second
            )
            data.append(label)
    ...
    def num_convert_duration(self, duration: float) -> float:
        ...
        if (
            (self.xaxis_dtype == "KiloMeter" and self.duration_dtype == "KiloMeter")
            or (self.xaxis_dtype == "Meter" and self.duration_dtype == "Meter")
            or (self.xaxis_dtype == "Number" and self.duration_dtype == "Number")
        ):
            res = float(duration)
    ...
    def plot_chart(self, tool="draw", fig_present=False):
        ...
        if self.choice_is_null:
            self.open_column_selection()
        ...
        try:
            ...
            if dtype_xaxis == "object":
                ...
                if self.xaxis_min != None:
                    ...
                    min = minvals[
                        self.xaxis_column
                    ]  # najwczesniejsza data wśród dat rozpoczęcia

                if self.xaxis_max != None:
                    if self.duration_dtype == "Week":
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            weeks=int(maxvals[self.duration_column])
                        )
                    elif self.duration_dtype == "Day":
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            days=int(maxvals[self.duration_column])
                        )
                    elif self.duration_dtype == "Hour":
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            hours=int(maxvals[self.duration_column])
                        )
                    elif self.duration_dtype == "Minute":
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            minutes=int(maxvals[self.duration_column])
                        )
                    elif self.duration_dtype == "Second":
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            seconds=int(maxvals[self.duration_column])
                        )
                    else:
                        max = pandas.to_datetime(self.xaxis_max) + datetime.timedelta(
                            minutes=int(maxvals[self.duration_column])
                    ...
                    max = maxvals[self.xaxis_column] + datetime.timedelta(
                        minutes=int(maxvals[self.duration_column])
                    )
                ...
                try:
                    for index, row in df.iterrows():
                        ...
                        if (
                            ...
                            or (
                                ...
                                and (
                                    pandas.to_datetime(start)
                                    < pandas.to_datetime(self.xaxis_min)
                                    or (
                                        pandas.to_datetime(start)
                                        > pandas.to_datetime(self.xaxis_max)
                                    )
                                )
                            )
                            or (
                    ...
                    plot_title = plt.title(
                        "Schedule",
                        font=self.chart_title_font,
                        fontsize=self.chart_title_fsize,
                    )
                    plt.setp(plot_title, color=self.chart_title_color)

                    if self.fig_bg:
                        self.fig.patch.set_facecolor(self.fig_bg)
            ...
            # =================================================================
                ...
                if self.xaxis_max:
                    ...
                    max = self.num_add_duration_to_start(
                        maxvals[self.xaxis_column], maxvals[self.duration_column]
                    )
                ...
                try:
                    for index, row in df.iterrows():
                        ...
                        if (
                            ...
                            or (
                                (self.xaxis_min != None and self.xaxis_max != None)
                                and (
                                    float(start) < float(self.xaxis_min)
                                    or (float(start) > float(self.xaxis_max))
                                )
                            )
                            or (
                                (self.zaxis_min != None and self.zaxis_max != None)
                        ...
                        if len(_y) == 0 or len(_z) == 0:
                            continue

                        y = _y[0]
                        z = _z[0]

                        xpos = self.num_add_duration_to_start(
                            float(start), float(duration) / 2
                        )
                        plot_pos = (xpos, y, z)
                        ...
                        self.plotCubeAt(
                            pos=plot_pos,
                            size=(self.num_convert_duration(duration), 0.1, 0.1),
                            color=obj_color,
                            ax=ax,
                        )
                    ...
                    if tool == "draw":
                        ...
                        self.draw_cut_chart(
                            datatype="numerical", figure_present=fig_present
                        )

                except Exception as e:
                    messagebox.showerror(
                        "Error ",
                        " Error while creating the chart \r\n" + traceback.format_exc(),
                    )

        except Exception as e:
            messagebox.showerror(
                "Error",
                "Error in calculations for the graph \r \n" + traceback.format_exc(),
            )
    ...
    def plot_intersections(self, intersections):
        ...
        stored_colors = self.color_cache.retrieve_cache()
        if self.marker_color in stored_colors:
            self.marker_color = stored_colors[self.marker_color]
        if self.aux_line_color in stored_colors:
            self.aux_line_color = stored_colors[self.aux_line_color]
    ...
    def open_column_selection(self):
        col_selection_window = ColumnSelectionWindow(
            self.adapter, dataframe=self.df, title="Column Selection", size=(1000, 500)
        )
    ...
    def open_custom_colors_window(self):
        ...
        if not self.adapter.ispresent("colorname"):
            pass
        else:
    ...
    def insert_new_column(self):
        ...
        self.df = pandas.DataFrame(
            self.sheet.get_sheet_data(), columns=self.sheet.headers()
        )
    ...
    def insert_row(self):
        ...
        insert_window = InsertRowWindow(
            self.adapter, df_cols, "Insert Row", size=(400, len(df_cols) * 50)
        )
        insert_window.start()
    ...
    def detect_collision(self):
        ...
        detector = CollisionDetectorExtended(
            dataframe=self.df,
            xstart=df[self.xaxis_column],
            xend=df["end"],
            space=self.zaxis_column,
            obj=self.yaxis_column,
        )
        collisions = detector.detect()
        report = detector.generate_report(collisions)

        self.report_window = CollisionReport(
            report=report,
            title="Collision Detector Report",
            size=(750, 500),
            font=self.sheet_font,
        )
